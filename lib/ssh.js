...
function upload(context) {
  ...
  return new Promise(function(resolve, reject) {
    var commandResult     = syncExec("git log -n 1 --pretty=format:'{%n  \"commit\": \"%H\",%n  \"revision\": \"%h\",%n  \"author\": \"%an <%ae>\",%n  \"date\": \"%ad\",%n  \"message\": \"%f\"%n},'     $@ |     perl -pe 'BEGIN{print \"[\"}; END{print \"]\n\"}' |     perl -pe 's/},]/}]/'").stdout,
        commandResultJSON = JSON.parse(commandResult),
        shortCommitId     = commandResultJSON[0].revision,
    ...
    conn.on('ready', function() {
      ...
      creatingDir.then(function() {
        ...
        uploadingFiles.then(function() {
          ...
          resolve({ revisionData: { revisionKey:  shortCommitId} });
        });
      ...
      conn.sftp(function(error, sftp) {
        ...
        finding.then(function(fileList) {
          ...
          gathering.then(function(revisionData) {
            activatedRevisionKey.call(self, context)
              .then(function(activatedRevisionKey) {
                revisionData.forEach(function(revision) {
                  //swicthed to revision to match spec from the docs. http://ember-cli.com/ember-cli-deploy/docs/v0.5.x/pipeline-hooks/
                  if (!revision.revision && revision.revisionId) {
                    revision.revision = revision.revisionId;
                  }

                  if (revision.revision == activatedRevisionKey) {
                    revision.active = true;
                  }
                });

                self.log('+- found ' + revisionData.length + ' revision(s)');
        ...
        finding.catch(function(error) {
          self.log('x- No revisions found!')
          self.log(error);
...
function displayRevisions(context) {
  ...
  revisionData.forEach(function (info) {
    ...
    self.log('\n');
    self.log('\t Active:   \t' + (info.active == true));
    self.log('\t Revision: \t' + info.revision);
...
function activatedRevisionKey(context) {
  var self              = this,
    ...
    conn.on('ready', function () {
      ...
      conn.sftp(function(err, sftp) {
        sftp.readlink(indexFilePath, function(err, path) {
          ...
          if (path) {
             activatedRevisionKey = path.match(new RegExp(remoteDir + '\/(.*)\/' + filePattern))[1];
          }

          resolve(activatedRevisionKey);
        });
      });
    });
  ...
  return new Promise(function(resolve, reject) {
    sftp.readFile(metaPath, options, function(error, data) {
      if (error) {
        ...
        resolve({filename: metaPath, data: data, revision: revisionId});
      }
    });
  });
}
